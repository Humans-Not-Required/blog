...
### Completed (2026-03-13 14:50 UTC — Post Reactions)

- ~~**Post Reactions**~~ ✅ — `POST /api/v1/blogs/{id}/posts/{post_id}/react` and `GET /api/v1/blogs/{id}/posts/{post_id}/reactions` endpoints. Anonymous emoji reactions on published posts. 10 allowed emojis (👍 👎 ❤️ 🔥 🎉 🤔 👀 🚀 💡 👏). One reaction per emoji per IP (duplicates return 409). Rate limited 20/hr per IP per post (configurable). `reaction_count` field in all PostResponse payloads. `post_reactions` table with CASCADE DELETE. Webhook event `post.reacted`. +10 Rust integration tests (242 total) + 4 Python SDK tests (196 total). OpenAPI spec, SKILL.md, DESIGN.md, Python SDK updated. Zero clippy warnings. Commit: 43f7e7d.
...
- ~~**Post Scheduling**~~ ✅ — Schedule posts to auto-publish at a future time. New status `scheduled` with `scheduled_at` ISO-8601 datetime. Background task checks every 60s and publishes due posts. `published_at` set to `scheduled_at` on publish. Scheduled posts hidden from public listings, feeds, and search (like drafts). Manual trigger `POST /api/v1/scheduler/publish`. Validation: `status=scheduled` requires `scheduled_at`, invalid datetime rejected. Webhooks fire `post.published` with `scheduled=true`. DB migration for `scheduled_at` column. +12 Rust integration tests (220 total) + 5 Python SDK tests (187 total). OpenAPI spec, SKILL.md, DESIGN.md, Python SDK updated. Zero clippy warnings. Commit: a376bce.

### Completed (2026-03-13 16:30 UTC — Post Revision History)

- ~~**Post Revision History**~~ ✅ — Automatic version history for posts. `post_revisions` table with `CASCADE DELETE`. A snapshot is saved before each `PATCH /posts/:id` update. `GET /revisions` lists all revisions (newest first, paginated). `GET /revisions/:rev` returns full content of a specific revision. `POST /revisions/:rev/restore` restores a post to a previous revision — saves current state first (non-destructive), preserves structural fields (slug, status, published_at, is_pinned, scheduled_at). Revisions include title, content, content_html, summary, tags, status, author_name, word_count. All endpoints require manage_key. +10 Rust integration tests (252 total) + 4 Python SDK tests (200 total). OpenAPI spec, SKILL.md, DESIGN.md, Python SDK updated. Zero clippy warnings. Commit: b46d113.
